...
- 書込み作業は feature branch 上でのみ行う。
- branch作成はOwner承認後に行う。
- `stage`、`commit`、`push`、`PR作成`、`merge` は別工程として扱い、それぞれ承認範囲を確認する。
- `main` / protected branch へ直接commitまたはpushしない。
- merge、production deploy、Figma final re-LOCK はOwner明示承認が必須。
- force push は現在のユーザーレベル Development Governance により禁止。Ownerの単発承認だけで迂回せず、例外には先に上位ガバナンスの明示変更が必要。
- `git reset --hard`、`git clean`、広範囲削除、親ディレクトリ掃除を行わない。
